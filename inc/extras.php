 ...
 * Add sidebars
 */
if (! function_exists('aeroblog_get_sidebar_layout') ) :
     ...
     */
    function aeroblog_get_sidebar_layout( $layout ) 
    {
 ...
 */
if (! function_exists('aeroblog_get_option') ) :
     ...
     */
    function aeroblog_get_option( $key = '', $defaults = '' ) 
    {
        $options = apply_filters(
            'aeroblog_theme_defaults_after_parse_args', wp_parse_args(
                get_option('aeroblog', true),
                aeroblog_get_defaults()
            )
        );
 ...
 * Add body classes
 */
if (! function_exists('aeroblog_body_class') ) :
     ...
     */
    function aeroblog_body_class( $classes ) 
    {

        if (is_home() || is_archive() || is_search() ) {
            $layout = aeroblog_get_option('sidebar-archive');
        } elseif (is_page() || is_404() ) {
            $layout = aeroblog_get_option('sidebar-page');
        } elseif (is_single() ) {
            $layout = aeroblog_get_option('sidebar-single');
        }
    ...
    add_filter('body_class', 'aeroblog_body_class');
 ...
 */
if (! function_exists('aeroblog_body_classes') ) :
     ...
     * @return array
     */
    function aeroblog_body_classes( $classes ) 
    {
    ...
    add_filter('body_class', 'aeroblog_body_classes');
 ...
 */
if (! function_exists('aeroblog_pingback_header') ) :

    /**
     * Add a pingback url auto-discovery header for singularly identifiable articles.
     */
    function aeroblog_pingback_header() 
    {
        if (is_singular() && pings_open() ) {
            echo '<link rel="pingback" href="', bloginfo('pingback_url'), '">';
        }
    }
    add_action('wp_head', 'aeroblog_pingback_header');
